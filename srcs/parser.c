...
/*   Updated: 2022/05/11 00:03:52 by merlich          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_fill_cmd(t_info *data)
{
	...
	while (data->token_head && data->token_head->type != PIPE)
	{
		...
		{
			data->token_head->next->str_val  // LIMITER (We have to create here_doc.tmp)
			data->token_head = data->token_head->next;
		}
		else if (data->token_head->type == REDIR_IN)
		{
			data->token_head->next->str_val  // infile (We have to check access)
			data->token_head = data->token_head->next;
		}
		else if (data->token_head->type == REDIR_OUT)
		{
		...
		else if (data->token_head->type == WORD we already have cmd)
...
void	ft_get_cmds(t_info *data)
{
	...
	head = data->group_head->cmds_head;
	data->token_head = data->tokens;
	while (data->token_head)
	{
		ft_cmd_lstadd_back(&head, ft_cmd_lstnew());
			// ft_fill_cmd(head, data->token_head);
				// while (data->token_head && data->token_head->type != PIPE)
				// {
				// 	data->token_head = data->token_head->next;
				// }
				// if (data->token_head->type == PIPE)
					// data->token_head = data->token_head->next;
	}
	if (data->tokens && ft_token_lstlast(data->tokens)->type == PIPE)
	{
		
		ft_cmd_lstadd_back(&head, ft_cmd_lstnew());
		// ft_fill_cmd(head);  // read cmd from stdin
	}
}
