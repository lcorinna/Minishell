...
/*   Updated: 2022/04/29 00:06:52 by merlich          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_replace(t_info *data)
{
	...
	printf("s2 = %s\n", s2);
	free(s1);
	s3 = ft_substr(s0, ind + 1 + ft_find_index(s0 + ind + 1), ft_strlen(s0 + ind + 1 + ft_find_index(s0 + ind + 1)));
	printf("d = %d\n", ft_find_index(s0 + ind + 1));
	printf("s3 = %s\n", s3);
...
static void	ft_interpret(t_info *data)
{
	...
	while (*str)
	{
		...
		if (str && (str[1] == '_' || ft_isalpha_big(str[1])))
		{
			...
			{
				printf("data->envp_head = %s\n", data->envp_head->key);
				if (!ft_strncmp(sub_str, data->envp_head->key, ft_strlen(sub_str) + ft_strlen(data->envp_head->key)))
				{
					ft_replace(data);
					break ;
				}
